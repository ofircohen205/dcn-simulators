...
class SharedMemoryWrapper(object):
    def __init__(self, folder: str, seed: int):
        self.shared_memory_python_path = osp.join(
            folder, f"shared_memory_{seed}_python.json"
        ).replace("\n", "")
        self.shared_memory_java_path = osp.join(folder, f"shared_memory_{seed}_java.json").replace(
            "\n", ""
        )
        self._max_retries = 5
        self._retry_count = 0
