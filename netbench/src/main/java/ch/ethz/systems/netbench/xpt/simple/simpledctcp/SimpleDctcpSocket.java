...
public class SimpleDctcpSocket extends SimpleTcpSocket {
     ...
     */
    protected SimpleDctcpSocket(TransportLayer transportLayer, long flowId, int sourceId, int destinationId, long flowSizeByte, long estimatedFlowSizeByte, NBProperties configuration, int jobId) {
        super(transportLayer, flowId, sourceId, destinationId, flowSizeByte, estimatedFlowSizeByte, configuration, jobId);
        this.DCTCP_WEIGHT_NEW_ESTIMATION = configuration.getDoublePropertyWithDefault(
                Constants.TCP.DCTCP_WEIGHT_NEW_ESTIMATION,
                0.0625);
