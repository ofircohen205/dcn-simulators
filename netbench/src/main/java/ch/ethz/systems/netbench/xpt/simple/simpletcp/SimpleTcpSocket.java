...
public class SimpleTcpSocket extends Socket {
    ...
    private static final long MAXIMUM_FLOW_SIZE = Long.MAX_VALUE;
    ...
    public SimpleTcpSocket(
            ...
            int destinationId,
            long flowSizeByte,
            long estimatedFlowSizeByte,
            NBProperties configuration,
            int jobId
    ) {
        super(transportLayer, flowId, sourceId, destinationId, flowSizeByte, estimatedFlowSizeByte, jobId);
        ...
        this.roundTripTimeout = configuration.getLongPropertyWithDefault(
                Constants.TCP.ROUND_TRIP_TIMEOUT_NS,
                1000000L);
        this.MINIMUM_ROUND_TRIP_TIMEOUT = configuration.getLongPropertyWithDefault(
                Constants.TCP.MINIMUM_ROUND_TRIP_TIMEOUT,
                1000000L);

        // Ethernet: 1500 - 60 (TCP header) - 60 (IP header) = 1380 bytes
        this.MAX_SEGMENT_SIZE = configuration.getLongPropertyWithDefault(
                Constants.TCP.MAX_SEGMENT_SIZE,
                1380L);

        // Conservative slow start threshold of 30 segments, which is 33120 bytes
        long INITIAL_SLOW_START_THRESHOLD = configuration.getLongPropertyWithDefault(
                Constants.TCP.INITIAL_SLOW_START_THRESHOLD,
                30 * MAX_SEGMENT_SIZE);

        // Maximum window size is 2^16-1 bytes, which is what is the maximum allowed in the TCP header
        this.MAX_WINDOW_SIZE = configuration.getLongPropertyWithDefault(
                Constants.TCP.MAX_WINDOW_SIZE,
                1073741824L);

        // Loss window is one segment (by default)
        this.LOSS_WINDOW_SIZE = configuration.getLongPropertyWithDefault(
                Constants.TCP.LOSS_WINDOW_SIZE,
                MAX_SEGMENT_SIZE);
        ...
        //
        long INITIAL_WINDOW_SIZE = configuration.getLongPropertyWithDefault(
                Constants.TCP.INITIAL_WINDOW_SIZE,
                3 * MAX_SEGMENT_SIZE);
...
//        System.out.println(this.congestionWindow);
    ...
    protected void cancelResendEvent(long seq) {
        ...
        event.cancel();
        Simulator.getEventQueue().remove(event);
