...
public class GraphReader {
    ...
    public static Pair<Graph, GraphDetails> read(String fileName) {
        ...
        try {
            ...
            // Go over parameter lines one-by-one, stop when encountering non-parameter
            // lines
            String line;
            while ((line = br.readLine()) != null) {
                ...
                if (index != -1) {
                    ...
                    switch (key) {
                        ...
                        // |V|=NUM_VERTICES
                        case "|V|":
                            details.setNumNodes(Integer.parseInt(val));
                            break;

                        // |E|=NUM_EDGES
                        case "|E|":
                            details.setNumEdges(Integer.parseInt(val));
                            break;
                        ...
                        case "AutoExtended":
                            if (val.equals("true")) {
                                ...
                                throw new IllegalArgumentException(
                                        "Cannot have value different than 'true' for AutoExtended");
                            }
                            break;
                        ...
                        case "Lanes":
                            details.setServerLanes(Integer.parseInt(val));
                            break;
                        case "MetaNodeNum":
                            details.setMetaNodeNum(Integer.parseInt(val));
                            break;
                        case "Cores":
                            details.setCoreNodeIds(convertCollectionOfIds(val));
                            break;
                        case "Aggregation":
                            details.setAggregationNodeIds(convertCollectionOfIds(val));
                            break;
                        default:
            ...
            br.close();
            Set<Integer> switchNodeIds = new HashSet<>();
            try {
                switchNodeIds.addAll(details.getAggregationNodeIds());
            } catch (NullPointerException e) {
                System.out.println("Aggregation nodes are not specified");
            }
            switchNodeIds.addAll(details.getCoreNodeIds());
            details.setSwitchNodeIds(switchNodeIds);
            ...
            if (details.getNumNodes() == -1 || details.getNumEdges() == -1 || details.getServerNodeIds() == null
                ...
                throw new IllegalArgumentException(
                        "One of the mandatory keys is missing: |V|, |E|, Servers, Switches, ToRs");
            }
            ...
            if (allJoin.size() != details.getNumNodes()) {
                throw new IllegalArgumentException(
                        "The ToRs, switches and servers together must cover the entire topology.");
            }
            ...
            if (!torServerIntersection.isEmpty() && !details.getTorNodeIds().equals(details.getServerNodeIds())) {
            ...
            details.setNumEdges(details.getNumEdges());
            while ((line = br.readLine()) != null) {
                line = line.trim();
                ...
                if (index == -1) {
                    ...
                    if (spl.length > 2) {
                        edgesDupNum = Long.parseLong(spl[2]);
                    }
            ...
            if (details.getNumEdges() != linkDirectedPairs.size()) {
                throw new IllegalArgumentException(
                        "The number of edges mentioned does not correspond to number of edges mentioned.");
            }
    ...
    /**
     * Converts a collection (e.g. set(INDICES), or incl_range(LOW, HIGH) into a
     * list of values.
     *
