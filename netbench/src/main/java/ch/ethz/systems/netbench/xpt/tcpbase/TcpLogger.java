...
public class TcpLogger implements LoggerCallback {
    ...
    public TcpLogger(long flowId, boolean isReceiver) {
        this.flowId = flowId;
        this.maxFlowlet = 0;
        this.congestionWindowWriter = SimulationLogger.getExternalWriter("congestion_window.csv");
        this.packetBurstGapWriter = SimulationLogger.getExternalWriter("packet_burst_gap.csv");
        this.maxFlowletWriter = SimulationLogger.getExternalWriter("max_flowlet.csv");
        this.logPacketBurstGapEnabled = Simulator.getConfiguration().getBooleanPropertyWithDefault(
                Constants.Logger.ENABLE_LOG_PACKET_BURST_GAP,
                false);
        this.logCongestionWindowEnabled = Simulator.getConfiguration().getBooleanPropertyWithDefault(
                Constants.Logger.ENABLE_LOG_CONGESTION_WINDOW,
                false);
